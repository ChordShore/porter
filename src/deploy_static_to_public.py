...
def deploy_static_to_public():
    ...
    create_public_directory(public_path)
    copy_static_directory(static_path, public_path)

def delete_public_directory(public_path):
    try:
...
def create_public_directory(public_path):
    try:
...
def copy_static_directory(static_path, public_path):
    file_list = os.listdir(static_path)
    for file in file_list:
        #print(file)
        static_copy = os.path.join(static_path, file)
        public_copy = os.path.join(public_path, file)
        #print(static_copy)
        #print(public_copy)
